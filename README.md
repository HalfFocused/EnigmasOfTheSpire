# Enigmas Of The Spire
An overly ambitious content mod for Slay the Spire 2

Enigmas Of The Spire adds three new characters to Slay the Spire 2, each with their own card pools, new mechanics, and stories.  
This mod is a Work In Progress!!! Anything and everything is subject to change.  
No AI was used in the creation of this mod... at all!!! And there never will be!!

## The Displaced
"A time-traveller, losing their memories in a loop. Witnesses visions and foresight of the future."  
Themes:
- Clinging onto their memories with powerful **Ethereal** cards.
- **Forget** cards at opportune moments to harness their unique effects when **Exhausted**.
- Seeing the future with their **Foretold** debuff that doubles the damage of the first attack against an enemy each turn.
- Harnessing **Visions**, powerful attack tokens that last only 3 turns.
- Sorting out their thoughts with clever card manipulation.
- Languishing in the debuffs upon their enemies.


## The Sacrifice
"An religious figure hoping to appease the Spire. Keeper of the Fyre, she shoulders a terrible secret."  
Themes:
- Her **Charred** debuff that increases received attack damage and halves each turn.
- Managing **Burn** statuses she creates through overexerting her power.
- Sticking to the simplicity and virtue of **Common** cards.
- Giving into the ego and allure of **Rare** cards.
- Keeping the energy high by drawing cards during her turns.
- Hiding her **Taboos**, retaining tokens that give draw when she need it most... at a price.

## The Savant
"An inventor banished from the Spire for his creations. Commands Chirp, his sentry companion."  
Themes:
- Charging **Chirp**, his trusty Sentry companion who can hold energy for him across turns.
- Utilizing powerful **Commands** that **Chirp** can spend it's energy on.
- Innovating and inventing during combat by **Transforming** cards into more useful ones.
- Building up **Scrap**, a unique status card that, when **Transformed**, makes the resulting card cost 0 until played.
- Be ready for anything by ending his turns with unspent energy for powerful bonuses.
- Spend massive amounts of saved-up energy on very powerful cards.
